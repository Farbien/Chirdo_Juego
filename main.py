...
pygame.mixer.music.set_volume(0.2) # Ajustar volumen (EMTRE 0.0 A 1.0)
...
def mostrar_texto(texto):
    ...
    render_texto = font.render(texto, True, COLOR_BLANCO)
    rect_texto = render_texto.get_rect(center=(ANCHO_VENTANA // 2, ALTO_VENTANA // 2))
    window.blit(render_texto, rect_texto)
    pygame.display.flip()
...
def comenzar():
    ...
    while running:
        ...
        if tiburon.tiempo == 80 or tiburon.vidas == 0:
            ...
            tiburon.puntos = 0
            tiburon.vidas = 5
            tiburon.tiempo = pygame.time.get_ticks()
            tiburon.nivel = 1
       ...
       # Almacenar valores
        ...
        tiburon.tiempo = pygame.time.get_ticks() // 1000
